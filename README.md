...
#include <pthread.h>
```
#### Membuat struct untuk keperluan pembuatan thread
```
...
int stopthread=0;
```
Digunakan sebagai flag agar thread yang sedang berjalan (looping) bisa selesai.

#### Fungsi yang akan dijalankan oleh para thread
```
...
```
Cara kerjanya adalah dengan mencocokkan waktu yang ada pada permintaan di file _crontab.data_ dengan waktu saat ini.

#### Bagian dari template pembuatan daemon
```
int main() {
  pid_t pid, sid;
...
```
  ...
  int t=0;
```
+ _oldMtime_ dan _nowMtime_ Untuk keperluan pengecekan apakah file _crontab.data_ diubah atau tidak.
+ _pertama_, variabel yang tidak digunakan tetapi belum dihapus.
+ _tid[100]_ dan _t_ untuk keperluan membuat thread
...
```
Intinya adalah untuk memeriksa apakah file _crontab.data_ pernah diubah dan membacanya lalu memecah-mecah tiap line dan kata untuk keperluan pembuatan thread.

#### Bagian akhir dari template pembuatan daemon
```
  exit(EXIT_SUCCESS);
}
```
